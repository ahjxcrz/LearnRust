...
function tsEnum(registry, definitions, {
  ...
  const keys = named.map(def => {
    ...
    const asGetter = type === 'Null' || info === TypeDefInfo.DoNotConstruct ? '' : createGetter(definitions, `as${getter}`, lookupName || (isComplex ? formatType(registry, definitions, info === TypeDefInfo.Struct ? def : type, imports, withShortcut) : type), imports);
  ...
  return exportInterface(lookupIndex, enumName, 'Enum', `${keys.join('')}  ${indent}readonly type: ${named.map(({
    name = ''
...
function tsResultGetter(registry, definitions, resultName = '', getter, def, imports) {
  ...
  const asGetter = type === 'Null' ? '' : createGetter(definitions, `as${getter}`, lookupName || (info === TypeDefInfo.Tuple ? formatType(registry, definitions, def, imports, false) : type), imports);
...
function tsStruct(registry, definitions, {
  ...
  const keys = sub.map(def => {
    const fmtType = def.lookupName && def.name !== def.lookupName ? def.lookupName : def.info === TypeDefInfo.Enum ? `${tsEnum(registry, definitions, def, imports, true)} & Enum` : formatType(registry, definitions, def, imports, false);
