...
function generateLookupTypes(registry, filtered, destDir, subPath) {
  ...
  const items = filtered.map(([, typeDef]) => {
    ...
    return typeDef.lookupNameRoot && typeDef.lookupName ? exportInterface(typeDef.lookupIndex, typeDef.lookupName, typeDef.lookupNameRoot) : typeEncoders[typeDef.info](registry, imports.definitions, typeDef, imports);
...
function generateRegistry(registry, filtered, destDir, subPath) {
  writeFile(path.join(destDir, `${subPath}.ts`), () => {
    const items = filtered.map(([, {
      lookupName
    }]) => lookupName).filter(n => !!n).sort().reduce((all, n) => all.includes(n) ? all : all.concat(n), []);
